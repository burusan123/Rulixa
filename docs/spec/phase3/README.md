...
Phase 2 までで `Rulixa` は単一 entry から高密度な局所地図を返せるようになったが、`RealWorkspace` のような大きな WPF ワークスペースを「システム全体として理解して説明する」用途では、複数 sub-map を束ねる仕組みが不足している。
...
`RealWorkspace` の `ShellViewModel` pack で、少なくとも次が読める状態を目標とする。
